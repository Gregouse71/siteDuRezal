...
export default function UserVerifyEmail() {
    ...
    const emailVerification = () => {
        try {
            accountService
                ...
                .then(() => {
                    popupService.changePopup({
                        status: "success",
                        message: "Verification de l'email réussie, redirection vers la connexion... ",
                    });
            ...
            popupService.changePopup({
                status: "error",
                message: "Informations de vérifications non reconnues dans le chemin d'accès rentré",
            });
        }
    };

    return <>Vous ne devriez pas voir cette page.</>;
}
