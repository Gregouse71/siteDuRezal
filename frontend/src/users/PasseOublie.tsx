...
export default function UserNewPassword() {
    ...
    const sendMail = () => {
        setPostAsk(true);
        accountService
            .getNewPasswordMail({ email: email })
            .then(() => {
                ...
                switch (errorStatus) {
                    case 400: {
                        popupService.changePopup({
                            status: "error",
                            message: "Cet adresse mail n'est pas enregistrée",
                        });
                        break;
                    }
                    default:
    ...
    return (
        <>
            ...
            {postAsk && (
                <div style={{ textAlign: "left", margin: "0 1vw 0 1vw" }}>
                    <p>
                        Vous avez demandé un nouveau mot de passe. Si on compte est associé à cette adresse mail, vous
