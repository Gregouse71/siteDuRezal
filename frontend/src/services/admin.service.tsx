...
export function useAdminService() {
    ...
    const modifyAccounts = (changesOnAccounts: any[]) => {
        const promiseArray = changesOnAccounts.map((change) => httpInstance.patch("users/" + change.uid, change));
    ...
    const deleteAccounts = (uidsToDelete: any[]) => {
        ...
        //     .catch(error =>  reject(error))
        // })
        const promiseArray = uidsToDelete.map((toDelete) => httpInstance.delete("users/" + toDelete));
        return Promise.allSettled(promiseArray);
    };
    ...
    const doesAccountVerifyAllFilters = (account: Account, userFilters: any) => {
        const userFiltersNames = Object.keys(userFilters).filter(
            (userFiltersName) => userFiltersName !== defaultFilterName,
        );
    ...
    const doesAccountVerifyFilter = (account: Account, filterName: string, filterValue: any) => {
        ...
        switch (filterName) {
            case "id":
                return accountFilteredFieldValue?.toString().includes(filterValue);
            case "acces_wifi":
                return accountFilteredFieldValue === filterValue;
            case "is_admin":
                return accountFilteredFieldValue === filterValue;
            case "prenom":
                return accountFilteredFieldValue?.includes(filterValue);
            case "nom":
                return accountFilteredFieldValue?.includes(filterValue);
            case "uid":
                return accountFilteredFieldValue?.includes(filterValue);
            ...
            case "t1PaymentType":
                return accountFilteredFieldValue === filterValue;
            case "t1PaidAt":
                return dateService.dateToString(accountFilteredFieldValue).includes(filterValue);
            case "cotizT2":
                return accountFilteredFieldValue === filterValue;
            case "t2PaymentType":
                return accountFilteredFieldValue === filterValue;
