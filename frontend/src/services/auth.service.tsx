...
export function useAuthService() {
    ...
    const login = (login: string, password: string) => {
        ...
        return new Promise<void>((resolve, reject) => {
            httpInstance
                .post("auth/token", params.toString(), {
                                ...
                                switch (errorMessage) {
                                    case "Incorrect username or password": {
                                        popupService.changePopup({
                                            status: "error",
                                            message: "Connexion refusée : mauvais login/mot de passe",
                                        });
                                        break;
                                    }
                                    case "Email not verified": {
                                        popupService.changePopup({
                                            status: "error",
                                            message: "Connexion refusée : email non vérifié",
                                        });
                                        break;
                                    }
                                    default:
                                        popupService.changePopup({
                                            status: "error",
                                            message: "Connexion refusée : raison inconnue",
                                        });
                                }
                                break;
                            }
        ...
        return (
            <>
                ...
                <div>
                    ...
                    {props.displayAccountCreationLink && (
                        <Link to="/resident/register" style={{ fontSize: "1rem", color: "blue" }}>
                            {" "}
                            Pas de compte ?{" "}
                        </Link>
                    )}
                    <br />
                    {props.displayAccountCreationLink && (
                        <Link to="/resident/new-password" style={{ fontSize: "1rem", color: "blue" }}>
                            {" "}
                            Mot de passe oublié ?{" "}
                        </Link>
                    )}
