    ...
    constructor(data: any) {
        ...
        this.t2PaymentType =
            data.t2PaymentType !== undefined
                ? paymentTypes.includes(data.t2PaymentType)
                    ? data.t2PaymentType
                    : paymentTypes[0]
                : paymentTypes[0];
        ...
        this.t3PaymentType =
            data.t3PaymentType !== undefined
                ? paymentTypes.includes(data.t3PaymentType)
                    ? data.t3PaymentType
                    : paymentTypes[0]
                : paymentTypes[0];
    ...
    differenceWith(otherAccount: Account | undefined) {
        if (otherAccount === undefined) return {};
            ...
            Object.keys(this)
                ...
                .forEach((key) => {
                    if (!isEqual((this as any)[key], (otherAccount as any)[key]))
                        (diff as any)[key] = (this as any)[key];
                });
            return diff;
        }
    }
            ...
            switch (trimesterSuffix) {
                ...
                case "PaidAt": {
                    if (newVal === null) {
                        ...
                        if (this.get(trimesterPrefix + "PaymentType") === paymentTypes[0])
                            this.simpleChange(trimesterPrefix + "PaymentType", paymentTypes[1]);
                    }
                    break;
                }
                case "PaymentType": {
