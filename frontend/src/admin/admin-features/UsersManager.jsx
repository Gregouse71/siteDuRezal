...
export default function UsersManager() {
    ...
    useEffect(() => {
        // Will only trigger when UsersManager is called for the first time
        adminService.updateDatabaseView().then(() => {});
    ...
    const onAccountValueChange = (id, field, newValue) => {
        ...
        if (account && !_.isEqual(account[field], newValue)) {
            if (["login", "email"].includes(field) && newValue) {
                if (
                    !window.confirm(
                        "Attention, modifier ce champ peut empecher un utilisateur d'accèder à son compte, confirmer ?",
                    )
                ) {
                    setCurrentAccouts(new Map(currentAccounts.set(id, account)));
                    return null;
                }
            }
            if (
                IdsSelected.length > 1 &&
                window.confirm("Appliquer ce changement sur les autres comptes sélectionnés ?")
            ) {
    ...
    const onCommitChanges = () => {
        if (window.confirm("Confirmer les changements demandés ?")) {
            var number_error = 0;
            adminService
                .modifyAccounts(arrayOfChanges)
                .then((accountModificationDataArray) => {
                    var isSucessForAll = true;
                    arrayOfChanges.forEach((IdAndAccount, index) => {
                    ...
                    if (isSucessForAll) {
                        ...
                        popupService.changePopup({
                            status: "error",
                            message: `Nombre d'erreurs : ${number_error}. Certains comptes n'ont pas pu être créés, voir la colonne 'message'`,
                        });
                    }
                })
                .catch((error) => {
    ...
    const onDeleteAccount = (id) => {
        if (window.confirm("Confirmer la suppression de ce compte ?")) {
            ...
            if (
                IdsSelected.length > 1 &&
                IdsSelected.includes(id) &&
                window.confirm("Effectuer cette action sur tous les comptes sélectionnés ? [Oui/Non]")
            ) {
            ...
            adminService.deleteAccounts(uidsToDelete).then((accountDeletionStatus) => {
                idsToDelete.forEach((idToDelete, index) => {
                    const account = currentAccounts.get(idToDelete);
                    if (account !== undefined) {
                        const message =
                            accountDeletionStatus[index].status === "fulfilled"
                                ? "SSuppression effectuée"
                                : "ESuppression non effectuée, raison : " + accountDeletionStatus[index].reason;
