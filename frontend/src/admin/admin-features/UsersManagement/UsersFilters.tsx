...
function FilterFieldNameSelection(props: any) {
    ...
    const possibleChoicesFieldName = conversionService.accountFieldsNameInEnglishWithoutPassword
        ...
        .map((fieldName) => (
            <option key={"choice filter name " + fieldName} value={fieldName}>
    ...
    return (
        <select
            ...
            onChange={(event) => props.setUserFilterName(filterFieldName, event.target.value)}
        >
            {
                <option value={filterFieldName}>
                    {conversionService.translateAccountFieldNameInFrench(filterFieldName)}{" "}
                </option>
            }
...
export default function UsersFilters(props: any) {
    ...
    const invertFilter = (field: string) => {
        props.setUserFilters({
            ...userFilters,
            [field]: { ...userFilters[field], inverted: !userFilters[field].inverted },
        });
    };
