...
export default function UserCreation() {
    ...
    const detectErrorsInUsers = (users: Account[]) => {
        ...
        const errorsMissingFields = users
            .map((user, index) => {
                ...
                if (somethingsMissing) {
                    return (
                        ...
                        index +
                        " : " +
                        (loginMissing ? "pas de login, " : "") +
                        (passwordMissing ? "pas de mot de passe, " : "") +
                        (emailMissing ? "pas d'email, " : "")
                    );
                } else {
                    return "";
                }
        ...
        const errorsLoginDuplicated =
            loginsDuplicated.length > 0
                ? loginsDuplicated.map((login) => {
                      return (
                          "Login " +
                          login +
                          " dupliqué dans les lignes " +
                          findIndexesOfDuplicatedValue(loginsNotNull, login).reduce(
                              (index, chars) => chars + ", " + index,
                              "",
                          )
                      );
                  })
        ...
        const errorsEmailDuplicated =
            emailsDuplicated.length > 0
                ? emailsDuplicated.map((email) => {
                      return (
                          "Email " +
                          email +
                          " dupliqué dans les lignes " +
                          findIndexesOfDuplicatedValue(emailsNotNull, email).reduce(
                              (index, chars) => chars + ", " + index,
                              "",
                          )
                      );
    ...
    const onCreationNewAccounts = () => {
        adminService
            ...
            .then((response) => {
                ...
                const newComputedUsers = users.map((account, index) => {
                ...
                if (isSucessForAll) {
                    ...
                    popupService.changePopup({
                        status: "warning",
                        message: "Succès, mais certains comptes n'ont pas pu être créés, voir la colonne 'message'",
                    });
                }
                ...
                if (usersSuccessfullyCreated.length > 0)
                    CSVService.exportAsCSV(usersSuccessfullyCreated, "Compte créés");
            })
            .catch((error) => {
    ...
    return (
        <>
            ...
            {errorsDetected.length > 0 && (
                <p style={{ color: "red" }}>
                    Erreurs détectées : {errorsDetected.length}
                    <ul>
                        {errorsDetected.map((error, index) => (
                            <li key={"Error detected user creation " + index}> {error} </li>
                        ))}
                    </ul>
                </p>
            )}
            ...
            <div style={{ display: "flex", justifyContent: "space-around" }}>
                ...
                <UserCreationCSV
                    setComputedUsersFromCSV={(computedUsers: Account[]) => setUsers([...users, ...computedUsers])}
                />
            </div>
        </>
    );
